...
function analyseScore() {
    ...
    // Calculate PPQ and initial tempo
    var ppq = origScore.division;  // PPQ (ticks per quarter note)
    ...
    for (n = 0; n < z; ++n) {
        ...
        var midiTick = Math.round((timeInSeconds * 1000000 * ppq) / tempo);
        tr = [timeInSeconds, d[0], Number(d[1].toFixed(2)), midiTick];
        lt = tt;
        rt.push(tr);
        currentTick = midiTick;  // Update currentTick for the next iteration
    }
