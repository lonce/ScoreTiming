...
MuseScore {
	...
	requiresScore: true;
	version: "1";
	menuPath: "Plugins. Export beat info2";
	...
	Component.onCompleted: {
		...
		if (mscoreMajorVersion >= 4) {
			exportLocationInfoPhys.title = "Export beat info2";
		}
	}

	MessageDialog {
		id: resultBox
		title: "Export beat info2"
...
function analyseBeat(cursor, analyseMap) {
	...
	function analyseScore() {
		...
		for (n = 0; n < z; ++n) {
			...
			tr = [(tt > lt ? tt : t) / 1000.0, d[0], Number(d[1].toFixed(2))];
			lt = tt;
			rt.push(tr);
		}
		...
		outfile.source = g_outputFolder + "/" + origScore.scoreName + ".beatinfo2.jsn";
